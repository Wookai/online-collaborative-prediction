classdef model_mf_gp_r < handle
    % Base class for all MF + GP(r) models
    % z_dn = v_d^T u_n + GP(m(x_d), k(x_d1, x_d2))
    ...
    methods(Abstract)
        ...
        [lik_hyp, mean_hyp, cov_hyp] = get_hyperparameters(m, hyp);
    ...
    methods
        ...
        function [y_hat_mu, y_hat_var] = predict(m, y, obs_idx, test_idx, varargin)
            ...
            if isempty(m.mu)
                m.mu = feval(m.mean{:}, m.hyp.mean, X);
                m.Sigma = m.sn2*feval(m.cov{:}, m.hyp.cov, X) + m.V*m.V';
            end
            ...
            alpha = solve_chol(L_Sigma_obs, y(obs_idx) - m.mu(obs_idx))/sl;
            y_hat_mu = m.mu(test_idx) + m.Sigma(obs_idx, test_idx)'*alpha;
            
            if nargout >= 2
                v = solve_chol(L_Sigma_obs, m.Sigma(obs_idx, test_idx))/sl;
                y_hat_var = diag(m.Sigma(test_idx, test_idx) - m.Sigma(obs_idx, test_idx)'*v);
            end
        end
